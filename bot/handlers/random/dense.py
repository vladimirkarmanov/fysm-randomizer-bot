...
from states.random import DenseModuleState
...
@router.callback_query(DenseModuleState.first_fysm_level, RandomCallback.filter(F.callback_name == 'fysm_level'))
async def first_fysm_level_callback(callback: types.CallbackQuery, callback_data: RandomCallback, state: FSMContext):
    ...
    await state.set_state(DenseModuleState.first_game_type)
...
@router.callback_query(DenseModuleState.first_game_type, RandomCallback.filter(F.callback_name == 'game_type'))
async def first_game_type_callback(callback: types.CallbackQuery, callback_data: RandomCallback, state: FSMContext):
    ...
    await state.set_state(DenseModuleState.second_fysm_level)
...
@router.callback_query(DenseModuleState.second_fysm_level, RandomCallback.filter(F.callback_name == 'fysm_level'))
async def second_fysm_level_callback(callback: types.CallbackQuery, callback_data: RandomCallback, state: FSMContext):
    ...
    await state.set_state(DenseModuleState.second_game_type)
...
@router.callback_query(DenseModuleState.second_game_type, RandomCallback.filter(F.callback_name == 'game_type'))
@db_session(commit=True)
async def second_game_type_callback(
    ...
    *,
    ...
    text = RandomizerService().get_random_practice(
        ...
        second_game_type=callback_data.game_type,
    )
